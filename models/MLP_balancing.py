...
def balancing_mlp_val(X, y, n_split_out=5,n_split_in=5, num_trials=5,random_state=42,balance : bool = False):
    # 1. Initialize the split
    scorer = make_scorer(average_precision_score, average='macro', response_method='predict_proba')
    all_results = []
...
def data_augmented_mlp(csv_data,root_dir, n_split=5, num_trials=5,random_state=42,balance : bool = False,encoder_name = 'perch2'):
    # 1. Initialize the split
    ...
    for i in range(num_trials) :
        ...
        model_params = {
            'MLP_Baseline': {
                'model': BalancedMLP(input_dim=1536,balanced=False),
                'params': {} # Plain Binary Cross Entropy
            }
        }
        #Cross validation techniques for inner and outer loop
        cv = MultilabelStratifiedKFold(n_splits=n_split, shuffle=True, random_state=i)
