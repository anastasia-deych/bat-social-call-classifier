"""
This module implements various configurations of Multilayer Perceptrons (MLPs) to address class imbalance in the bat call classification task.
"""
...
def balancing_mlp_val(X, y, n_split_out=5,n_split_in=5, num_trials=5,random_state=42):
    """Performs nested cross-validation with hyperparameter tuning for multiple MLP configurations to address class imbalance.
    With validation data passed for tracking loss during MLP training.
...
def data_augmented_mlp(csv_data,root_dir,X_perch,y_perch, n_split=5, num_trials=5,random_state=42):
    """
    This function implements a data-augmented, time-shifted MLP training and evaluation pipeline with cross-validation.
    csv_data: Path to the CSV file containing metadata and labels.
    root_dir: Root directory where the audio files are stored.
    X_perch, y_perch: Pre-extracted features and labels from an encoder for the entire dataset.
    n_split: Number of folds for cross-validation.
    num_trials: Number of random trials to perform for robust evaluation.
    random_state: Base random seed for reproducibility, which will be varied across trials.
    """
    # 1. Initialize the split
